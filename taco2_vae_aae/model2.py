...
class Speaker_Encoder(nn.Module):
        ...
        for i in range(2):
            if i == 0:
                conv_layer = nn.Sequential(
                    ConvNorm2d(1 if i == 0 else 512,
                             hparams.reference_dim,
                             kernel_size=(3, 3), stride=(2, 2),
                             padding=(1, 1),
                             dilation=1, w_init_gain='relu', coord=False),
                    nn.BatchNorm2d(512))
            elif i == 1:
                conv_layer = nn.Sequential(
                    ConvNorm2d(hparams.reference_dim, hparams.reference_dim,
                             kernel_size=(3, 3), stride=(2, 2),
                             padding=(1, 1),
                             dilation=1, w_init_gain='relu', coord=True),
                    nn.BatchNorm2d(hparams.reference_dim))
            convolutions.append(conv_layer)
        self.convolutions = nn.ModuleList(convolutions)
        out_channels = self.calculate_channels(hparams.n_mel_channels, 3, 2, 1, 2)
        self.lstm = nn.LSTM(hparams.reference_dim*out_channels, hparams.reference_dim//2, 1, batch_first=True, bidirectional=True)
    ...
    def inference(self, x):
        N = 1
        x = x.contiguous().view(N, 1, -1, self.n_mels)

        for conv in self.convolutions:
            x = F.relu(conv(x.cuda()))
        x = x.transpose(1, 2)
...
class augmentation_classifier(nn.Module):
 ...
 # with tf.variable_scope('AAE') as scope:
        #
        #     y = self.aug_label
        #     batchs = 8
        #     z_id_ = np.random.randint(0, 3, size=[batchs])
        #     z_prior_ = gaussian_mixture(batch_size=batchs, n_dim=hp.z_residual_dim, n_labels=3, x_var=0.5, y_var=0.1, label_indices=z_id_)
        #
        #     z_prior_ = tf.convert_to_tensor(z_prior_, dtype=tf.float32)
        #     z_id = tf.one_hot(z_id_, 3)
        #
        #     z_prior = tf.concat([z_prior_, z_id], axis=1)
        #     z_fake = tf.concat([self.pr_z, y], axis=1)
        #
        #     self.D_real_logits = self.discriminator(z_prior)
        #     self.D_fake_logits = self.discriminator(z_fake)

class VAE(nn.Module):
    def __init__(self, hparams):
        super(VAE, self).__init__()
        self.fc_s_mu = LinearNorm(hparams.reference_dim, hparams.z_speaker_dim)
        self.fc_s_lv = LinearNorm(hparams.reference_dim, hparams.z_speaker_dim)

        self.fc_r_mu = LinearNorm(hparams.reference_dim, hparams.z_residual_dim)
        self.fc_r_lv = LinearNorm(hparams.reference_dim, hparams.z_residual_dim)
    ...
    def forward(self, inputs, speaker_z=True):
        if speaker_z==True:
            mu = self.fc_s_mu(inputs)
            logvar = self.fc_s_lv(inputs)
    ...
    def inference(self, inputs, ref_mu, ref_logvar, speaker_z=True, condition_on_ref=True):
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder_embedding_dim = hparams.encoder_embedding_dim + hparams.z_speaker_dim + hparams.z_residual_dim
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.augmenation_class = augmentation_classifier()
        self.vae = VAE(hparams)

    def parse_batch(self, batch):
        text_padded, input_lengths, mel_padded, gate_padded, \
            output_lengths, speaker_id ,labels = batch
        text_padded = to_gpu(text_padded).long()
        input_lengths = to_gpu(input_lengths).long()
        speaker_id = to_gpu(speaker_id).float()
        labels = to_gpu(labels).float()
        ...
        return (
            ...
            (mel_padded, gate_padded, speaker_id,labels))
    ...
    def forward(self, inputs):
        ...
        speaker_encoding, s_mu, s_lv  = self.vae(self.speaker_encoder(targets, output_lengths))
        residual_encoding, r_mu, r_lv  = self.vae(self.residual_encoder(targets, output_lengths), speaker_z=False)
        ...
        return self.parse_output(
            [mel_outputs, mel_outputs_postnet, gate_outputs, alignments, s_mu, s_lv, r_mu, r_lv, speaker_output, augmentation_output],
            output_lengths)


    def inference(self, inputs, refence_mel, ref_length, condition_on_ref=True, mu=0, logvar=1):
        inputs = self.parse_input(inputs)
        embedded_inputs = self.embedding(inputs).transpose(1, 2)

        speaker_encoding = self.vae.inference(self.speaker_encoder.inference(refence_mel), mu, logvar)    ### vae에서의 z가 speaker_encoding

        if condition_on_ref:
            residual_encoding = self.vae.inference(self.residual_encoder.inference(refence_mel), mu, logvar, speaker_z=False, condition_on_ref=True)
        else:
            residual_encoding = self.vae.inference(self.residual_encoder.inference(refence_mel), mu, logvar, speaker_z=False, condition_on_ref=False)
