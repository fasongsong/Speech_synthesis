...
def prepare_dataloaders(hparams):
    ...
    warp_trainset = TextMelLoader(hparams.training_files, hparams, warp_set=True)
    ...
    train_loader = DataLoader(torch.utils.data.ConcatDataset([trainset, warp_trainset]), num_workers=1, shuffle=True,
                              sampler=None,
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            y_pred = model(x)
            loss, recon_loss, S_kl_loss, R_kl_loss, speaker_loss, augment_loss, alignment_loss = criterion(y_pred, y, iteration)

            #########################speaker, augmentation classifier accuracy #######################################
            _,_, _, _, _,_, _, _, speaker_out, aug_out = y_pred
