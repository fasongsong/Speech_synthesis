...
from loss_function_vae import Tacotron2Loss
from logger_vae import Tacotron2Logger
...
def validate(model, criterion, valset, valset_aug_time, valset_aug_freq, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        if distributed_run:
            ...
            val_loader = DataLoader(torch.utils.data.ConcatDataset([valset, valset_aug_time, valset_aug_freq]),
                                    sampler=None, num_workers=35,
        ...
        for i, batch in enumerate(val_loader):
            ...
            y_pred = model(x)
            loss, recon_loss, R_kl_loss, R_kl_weight, speaker_loss, augment_loss, alignment_loss = criterion(y_pred, y, iteration)

            #########################speaker, augmentation classifier accuracy #######################################
            _, _, _, _, _, _, speaker_out, aug_out = y_pred
            speaker_target, aug_target = y[2], y[3]
            for e in range(0, hparams.batch_size):
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if not overflow and not math.isnan(reduced_loss) and rank == 0:
                ...
                logger.log_training(reduced_loss, grad_norm, learning_rate, duration, recon_loss, R_kl_loss, R_kl_weight, \
                                    speaker_loss, augment_loss, alignment_loss, iteration)

            if not overflow and (iteration % hparams.iters_per_checkpoint == 0):
                validate(model, criterion, valset, valset_aug_time, valset_aug_freq, iteration,
