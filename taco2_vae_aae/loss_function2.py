...
class Tacotron2Loss(nn.Module):
    ...
    def forward(self, model_output, targets, step):         ########## target에 og / aug 구분 라벨추가 코드필요
        ...
        mel_out, mel_out_postnet, gate_out, alignment, s_mu, s_lv, r_mu, r_lv, speaker_out, aug_out = model_output
        ...
        S_kl_loss = -0.5 * torch.sum(1 + s_lv - s_mu.pow(2) - s_lv.exp())
        R_kl_loss = -0.5 * torch.sum(1 + r_lv - r_mu.pow(2) - r_lv.exp())

        # S_kl_weight = self.kl_anneal_function(self.anneal_function, self.lag, step, self.k, self.x0, self.upper)
        # R_kl_weight = self.kl_anneal_function(self.anneal_function, self.lag, step, self.k, self.x0, self.upper)
        S_kl_weight = self.kl_anneal_function(self.anneal_function, step, self.k, self.x0)
        ...
        total_loss = 10 * (mel_loss + gate_loss) + 0.01 * (S_kl_loss*S_kl_weight + R_kl_loss*R_kl_weight) + 0.1 * speaker_loss + 0.1 * augment_loss
        #total_loss = mel_loss + gate_loss + S_kl_loss*S_kl_weight + R_kl_loss*R_kl_weight + speaker_loss + augment_loss

        return total_loss, tmp_loss, S_kl_loss, R_kl_loss, speaker_loss, augment_loss, alignment_loss
