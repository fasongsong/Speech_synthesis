...
from prior_factor import gaussian, gaussian_mixture
import numpy as np
...
class Residual_Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths):
        ...
        z_r = self.z_r(outputs)

        return z_r

    def inference(self, x, logvar=0):
        N = 1
        x = x.contiguous().view(N, 1, -1, self.n_mels)
...
class Discriminator(nn.Module):
    ...
    def forward(self, z_r, y):

        #############  Discriminator 용 #############
        z_id_ = np.random.randint(0, 3, size=[self.batchs])    ### 체크해야함.
        z_prior_ =gaussian_mixture(batch_size=self.batchs, n_dim=self.z_r_dim, n_labels=3, x_var=0.5, y_var=0.1, label_indices=z_id_)

        z_id = F.one_hot(torch.as_tensor(z_id_), num_classes=3)
        z_id = z_id.type(dtype=torch.float32).cuda()
        z_prior_ = torch.as_tensor(z_prior_).cuda()
        z_prior = torch.cat([z_prior_, z_id], dim=1)
        z_fake = torch.cat([z_r, y], dim=1)   ##### dim 확인하기.

        prior = F.dropout(F.relu(self.aae_hidden1(z_prior)))
        prior = F.dropout(F.relu(self.aae_hidden2(prior)))
        D_real_logits = self.aae_output(prior)

        fake = F.dropout(F.relu(self.aae_hidden1(z_fake)))
        fake = F.dropout(F.relu(self.aae_hidden2(fake)))
        D_fake_logits = self.aae_output(fake)

        return D_real_logits, D_fake_logits
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.speaker_claasifier = speaker_classifier(hparams)
        self.augmentation_classifier = augmentation_classifier(hparams)
    ...
    def forward(self, inputs):
        ...
        input_lengths, output_lengths = input_lengths.data, output_lengths.data           ## label이랑 length들 타입비교.

        embedded_inputs = self.embedding(inputs).transpose(1, 2)

        speaker_encoding = self.speaker_encoder(targets, output_lengths)
        speaker_out = self.speaker_claasifier(speaker_encoding)
        augment_out = self.augmentation_classifier(speaker_encoding)
        residual_encoding = self.residual_encoder(targets, output_lengths)

        ######### AAE #######
        D_real_logits, D_fake_logits = self.discriminator(residual_encoding, warp_labels)
        #####################
        ...
        return self.parse_output(
            [mel_outputs, mel_outputs_postnet, gate_outputs, alignments, speaker_out, augment_out, D_real_logits, D_fake_logits],
            output_lengths)


    def inference(self, inputs, refence_mel, ref_length, condition_on_ref=True, logvar=0):
        inputs = self.parse_input(inputs)
        embedded_inputs = self.embedding(inputs).transpose(1, 2)

        speaker_encoding = self.speaker_encoder.inference(refence_mel)    ### vae에서의 z가 speaker_encoding

        if condition_on_ref:
            residual_encoding = self.residual_encoder.inference(refence_mel, logvar)
        else:
            residual_encoding = self.residual_encoder.inference(refence_mel)
