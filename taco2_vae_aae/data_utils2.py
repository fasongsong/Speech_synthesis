...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, audiopaths_and_text, hparams, warp_set=False):
    ...
    def get_mel_text_pair(self, audiopath_and_text):
        ...
        org_label = torch.LongTensor([1,0])
        #Data augmentation                                         #### time  or  freq warping  라벨 구분 추후 실험.

        if self.mel_time_warping:
            warping_mel = SpecAugment.time_warping(mel, 0.3)
            aug_label = torch.LongTensor([0,1])
        if self.mel_freq_warping:
            warping_mel = SpecAugment.freq_warping(mel, 4.0)
            aug_label = torch.LongTensor([0,1])


        if self.warp_set == True:
            return (text, warping_mel, speaker_id, aug_label)
        else:
            return (text, mel, speaker_id, org_label)
...
class TextMelCollate():
    ...
    def __call__(self, batch):
        ...
        labels = torch.LongTensor(len(batch), len(batch[0][3]))
        for i in range(len(ids_sorted_decreasing)):
            label = batch[ids_sorted_decreasing[i]][3]
            labels[i, :] = label
        ...
        return text_padded, input_lengths, mel_padded, gate_padded, \
            output_lengths, speakers, labels
# import random
# import numpy as np
# import torch
# import torch.utils.data
